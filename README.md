...
- # JMeter TS/TJ Time Measurement Report  
  | **Single-instance Version Test Plan**          | **Graph Results Screenshot** | **Average Query Time(ms)** | **Average Search Servlet Time(ms)** | **Average JDBC Time(ms)** | **Analysis** |
    |------------------------------------------------|------------------------------|----------------------------|-------------------------------------|---------------------------|--------------|
    | Case 1: HTTP/1 thread                          | ![Link](https://github.com/UCI-Chenli-teaching/cs122b-spring20-team-191/blob/master/img/single_case1.png)   | 28                         | 1.04                                  | 0.77                       | This case shows the fastest performance because there are no other connections to share the core of the computer.           |
    | Case 2: HTTP/10 threads                        | ![Link](https://github.com/UCI-Chenli-teaching/cs122b-spring20-team-191/blob/master/img/single_case2.png)   | 28                         | 2.53                                  | 1.30                        | This case is slower than the Case 1. This is because AWS t2.micro free tier service has only one core. Other threads have to wait until a thread is over. However, the average query time is about the same as Case 1. If there are more cores given, multi-threading will provide better performance.           |
    | Case 3: HTTPS/10 threads                       | ![Link](https://github.com/UCI-Chenli-teaching/cs122b-spring20-team-191/blob/master/img/single_case3.png)   | 39                         | 8.10                                  | 5.18                    | HTTPS is always slower than HTTP in the same environment because it has encryption/decryption processes.           |
    | Case 4: HTTP/10 threads/No connection pooling  | ![Link](https://github.com/UCI-Chenli-teaching/cs122b-spring20-team-191/blob/master/img/single_case4.png)   | 27                         | 2.60                                  | 1.33                        | HTTP with no connection pooling is a little slower than HTTP with connection pooling because creating new connections for every request takes longer.          |

    | **Scaled Version Test Plan**                   | **Graph Results Screenshot** | **Average Query Time(ms)** | **Average Search Servlet Time(ms)** | **Average JDBC Time(ms)** | **Analysis** |
    |------------------------------------------------|------------------------------|----------------------------|-------------------------------------|---------------------------|--------------|
    | Case 1: HTTP/1 thread                          | ![](https://github.com/UCI-Chenli-teaching/cs122b-spring20-team-191/blob/master/img/scaled_case1.png)   | 27                         | 2.33                                 | 1.33                       | As the single instance test shows, a single thread is faster than multiple threads in the scaled version.           |
    | Case 2: HTTP/10 threads                        | ![](https://github.com/UCI-Chenli-teaching/cs122b-spring20-team-191/blob/master/img/scaled_case2.png)   | 30                         | 4.06                                 | 2.23                        | The case2 shows slower performance than case1. Because of the one core of AWS instance, other threads have to wait when a thread is working.            |
    | Case 3: HTTP/10 threads/No connection pooling                        | ![](https://github.com/UCI-Chenli-teaching/cs122b-spring20-team-191/blob/master/img/scaled_case3.png)   | 33                         | 5.18                                 | 2.81                        | This case is the slowest one in the Scaled version test. Since the program has to create a connection for every request, it takes longer to finish the works of search servlets and JDBC connections.           |        
      
    * Results may vary depending on the status of AWS instances. The existing Fabflix AWS instance was used for single instance 8443 case, and the rest were tested on new AWS instances (load balancer, master, slave, GCP load balancer)
